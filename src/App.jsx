...
function TodayMatches(props){
  ...
  useEffect(function(){
    async function fetchLive(){
      var data=await apiCall('fixtures?league='+WC_LEAGUE+'&season='+WC_SEASON+'&date='+new Date().toISOString().slice(0,10))
      if(!data)return
      var live={}
      data.forEach(function(f){
        var s=f.fixture.status.short
        var isActive=s==='1H'||s==='2H'||s==='HT'||s==='ET'||s==='FT'||s==='AET'||s==='PEN'
        if(!isActive)return
        var ha=f.goals.home,hb=f.goals.away
        if(ha===null||hb===null)return
        var homeName=API_TEAM_MAP[f.teams.home.name]||f.teams.home.name
        var awayName=API_TEAM_MAP[f.teams.away.name]||f.teams.away.name
        var isFinished=s==='FT'||s==='AET'||s==='PEN'
        var isLive=s==='1H'||s==='2H'||s==='HT'||s==='ET'
        GROUP_MATCHES.forEach(function(gm){
          var g=GROUPS.find(function(x){return x.name===gm.g})
          if(!g)return
          var ta=g.teams[gm.a].n,tb=g.teams[gm.b].n
          if(homeName===ta&&awayName===tb){
            live[gm.id]={a:String(ha),b:String(hb),status:s,elapsed:f.fixture.status.elapsed,isLive,isFinished}
          }
        })
      })
      setLiveScores(live)
