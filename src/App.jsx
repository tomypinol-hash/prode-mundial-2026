...
function calcScore(prode,results){
  ...
  rounds.forEach(function(r){
    ...
    // Union de ids con ganador elegido O con marcador cargado, para no perder puntos de marcador
    // exacto cuando el jugador predijo un empate pero no llego a elegir ganador de penales
    var allIds=Object.assign({},rd,ks)
...
// Aviso al admin: jugadores que todavia no cargaron pronostico para partidos
// que cierran dentro de la proxima hora (WARNING_MINS)
var WARNING_MINS=60
function getUpcomingLockWarnings(players,now){
  var warnings=[]
  var activePlayers=(players||[]).filter(function(p){return p.player_name!==ADMIN})
  // Partidos de grupos
  GROUP_MATCHES.forEach(function(m){
    var lockTime=new Date(m.date.getTime()-LOCK_MINS*60000)
    var minsToLock=(lockTime-now)/60000
    if(minsToLock<=0||minsToLock>WARNING_MINS)return
    var missing=activePlayers.filter(function(p){
      var sc=p.prode&&p.prode.scores&&p.prode.scores[m.id]
      return !sc||sc.a===''||sc.b===''||sc.a===undefined||sc.b===undefined||sc.a===null||sc.b===null
    }).map(function(p){return p.player_name})
    if(missing.length>0){
      var g=GROUPS.find(function(x){return x.name===m.g})
      var label='Grupo '+m.g+': '+g.teams[m.a].n+' vs '+g.teams[m.b].n
      warnings.push({id:m.id,label:label,lockTime:lockTime,missing:missing})
    }
  })
  // Partidos eliminatorios (16avos a semis)
  var koRounds=['r32','r16','qf','sf'];
  koRounds.forEach(function(round){
    ...
    for(var idx=0;idx<count;idx++){
      ...
      var d=MATCH_DATES[id]
      if(!d)continue
      var lockTime=new Date(d.getTime()-LOCK_MINS*60000)
      var minsToLock=(lockTime-now)/60000
      if(minsToLock<=0||minsToLock>WARNING_MINS)continue
      var missing=activePlayers.filter(function(p){
        var w=p.prode&&p.prode[round]&&p.prode[round][id]
        return !w
      }).map(function(p){return p.player_name})
      if(missing.length>0){
        warnings.push({id:id,label:(ROUND_LABELS[round]||round)+' '+(idx+1),lockTime:lockTime,missing:missing})
      }
    }
  })
  // Final
  ...
  if(finalD){
    var finalLock=new Date(finalD.getTime()-LOCK_MINS*60000)
    var minsToLock=(finalLock-now)/60000
    if(minsToLock>0&&minsToLock<=WARNING_MINS){
      var missing=activePlayers.filter(function(p){
        var w=p.prode&&p.prode.final&&p.prode.final.final_m
        return !w
      }).map(function(p){return p.player_name})
      if(missing.length>0)warnings.push({id:'final_m',label:'Final',lockTime:finalLock,missing:missing})
    }
  }
  warnings.sort(function(a,b){return a.lockTime-b.lockTime})
  return warnings
}
...
export default function App(){
  ...
  if(screen==='prode'){
    ...
    var myWarnings=getUpcomingLockWarnings([{player_name:currentPlayer,prode:prode}],now)
    return(
      <div style={{maxWidth:480,margin:'0 auto',padding:'8px'}}>
        <div style={sHeader}>
          <div style={{fontSize:11,opacity:.8}}>Prode de</div>
        ...
        {myWarnings.length>0&&(
          <div style={{background:'#fff3e0',border:'2px solid '+C.gold,borderRadius:10,padding:'10px 12px',marginBottom:8,fontSize:12}}>
            <div style={{fontWeight:700,color:'#e08600',marginBottom:6}}>⏰ ¡Te falta cargar un pronóstico!</div>
            {myWarnings.map(function(w){
              var tl=timeLeftStr(new Date(w.lockTime.getTime()+LOCK_MINS*60000))
              return(<div key={w.id} style={{padding:'3px 0',color:'#a06a00'}}>
                <span style={{fontWeight:600,color:C.blue}}>{w.label}</span>{tl?' — cierra en '+tl:' — a punto de cerrar'}
              </div>)
            })}
          </div>
        )}
    ...
    (function(idx){
      ...
      items.push(<div key={id} style={Object.assign({},sCard,{marginBottom:8})}>
        ...
        {realScore&&(function(){
          ...
          var ajusteManual=parseInt(prode.ajustesManuales&&prode.ajustesManuales[id])||0
          var total=ptsGanador+ptsExacto+ajusteManual
          ...
          if(ajusteManual)partes.push('+'+ajusteManual+'pt (ajuste admin)')
...
function FinalTab(props){
  ...
  if(realFinalWinner){
    ...
    var acertoGanadorFinal=champ&&champ.n===realFinalWinner.n
...
function AdminPanel(props){
  ...
  var lockWarnings=getUpcomingLockWarnings(players,now)
  var [saving,setSaving]=useState(false)
  var sorted=[...players].sort(function(a,b){return calcScore(b.prode,results)-calcScore(a.prode,results)})
  ...
  return(
    <div style={{maxWidth:600,margin:'0 auto',padding:'8px'}}>
      ...
      {lockWarnings.length>0&&(
        <div style={{background:'#fff3e0',border:'2px solid '+C.gold,borderRadius:10,padding:'10px 12px',margin:'8px 0',fontSize:12}}>
          <div style={{fontWeight:700,color:'#e08600',marginBottom:6,display:'flex',alignItems:'center',gap:6}}>
            ⚠️ Faltan pronósticos — cierran pronto
          </div>
          {lockWarnings.map(function(w){
            var tl=timeLeftStr(new Date(w.lockTime.getTime()+LOCK_MINS*60000))
            return(
              <div key={w.id} style={{padding:'5px 0',borderBottom:'1px solid rgba(0,0,0,.06)'}}>
                <div style={{fontWeight:600,color:C.blue}}>{w.label} {tl?'— cierra en '+tl:''}</div>
                <div style={{color:'#a06a00'}}>Faltan: {w.missing.join(', ')}</div>
              </div>
            )
          })}
        </div>
