  ...
  return(
    <div>
      ...
      <div style={{background:'#fff',border:'1px solid '+C.border,borderRadius:'0 0 10px 10px',padding:'10px 8px'}}>
        ...
        {activeTab==='r32'&&<KnockoutTab round='r32' prode={prode} results={results} setProde={setProde} readonly={readonly} realStandings={realStandings}/>}
        {activeTab==='r16'&&<KnockoutTab round='r16' prode={prode} results={results} setProde={setProde} readonly={readonly} realStandings={realStandings}/>}
        {activeTab==='qf'&&<KnockoutTab round='qf' prode={prode} results={results} setProde={setProde} readonly={readonly} realStandings={realStandings}/>}
        {activeTab==='sf'&&<KnockoutTab round='sf' prode={prode} results={results} setProde={setProde} readonly={readonly} realStandings={realStandings}/>}
        {activeTab==='final'&&<FinalTab prode={prode} results={results} setProde={setProde} readonly={readonly} realStandings={realStandings}/>}
      </div>
    </div>
  )
}
...
function KnockoutTab(props){
  var round=props.round,prode=props.prode,results=props.results,setProde=props.setProde,readonly=props.readonly,realStandings=props.realStandings
  ...
  for(var i=0;i<count;i++){
    (function(idx){
      ...
      // resultado real del partido eliminatorio
      var realWinner=results&&results[id]||null
      var realScore=results&&results[id+'_score']||null
      // calcular puntos: 1 si acerto equipo, doble si ademas acerto marcador
      ...
      items.push(<div key={id} style={Object.assign({},sCard,{marginBottom:8})}>
        ...
        {[ta,tb].map(function(t,ti){
          ...
          return(<div key={ti} style={Object.assign({},sMatchTeam(isW),{cursor:(locked||!t||readonly)?'default':'pointer',background:isRealW?'#eafff0':isW?'#e3f0fb':'#fff'})} onClick={function(){if(t&&!locked&&!readonly){var nr=Object.assign({},prode[round],{[id]:t});setProde(Object.assign({},prode,{[round]:nr}))}}}>
