...
const s = {
  header: {
    background: 'linear-gradient(135deg,#C0392B 0%,#1565C0 60%,#F9A825 100%)',
    padding: '16px', textAlign: 'center', borderRadius: '14px', marginBottom: '8px', color: '#fff'
  },
  card: {
  ...
  tab: function(a) {
    return {
      flex: 1, padding: '9px 2px', border: 'none', borderRadius: '8px 8px 0 0',
      cursor: 'pointer', fontSize: '11px', fontWeight: 500,
      background: a ? C.blue : '#eee', color: a ? '#fff' : '#555'
    }
  },
  teamRow: function(sel) {
    return {
      display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 10px',
      cursor: 'pointer', fontSize: '13px', borderBottom: `1px solid ${C.border}`,
      background: sel === 1 ? '#fff8e1' : sel === 2 ? '#e8f5e9' : '#fff',
      borderLeft: sel === 1 ? `4px solid ${C.gold}` : sel === 2 ? `4px solid ${C.green}` : '4px solid transparent'
    }
  },
  matchTeam: function(w) {
    return {
      display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 10px',
      cursor: 'pointer', fontSize: '13px', borderBottom: `1px solid ${C.border}`,
      background: w ? '#e3f0fb' : '#fff',
      borderLeft: w ? `4px solid ${C.blue}` : '4px solid transparent',
      fontWeight: w ? 600 : 400
    }
  },
  btn: function(col) {
    return {
      background: col || C.blue, color: '#fff', border: 'none',
      borderRadius: '10px', padding: '10px 20px', cursor: 'pointer',
      fontSize: '14px', fontWeight: 500, width: '100%', marginTop: '8px'
    }
  },
  input: {
    width: '100%', padding: '12px 14px', fontSize: '18px',
    border: `2px solid ${C.blue}`, borderRadius: '12px',
    outline: 'none', fontFamily: 'inherit', textAlign: 'center'
  },
  lockBadge: {
    display: 'inline-block', background: '#ffecb3', color: '#7b5800',
...
export default function App() {
  ...
  if (screen === 'ranking') {
    ...
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '12px 8px' }}>
        ...
        <button onClick={fetchPlayers} style={s.btn(C.blue)}>🔄 Actualizar ranking</button>
        <div style={{ ...s.card, marginTop: 8 }}>
          {ranked.length === 0 && <div style={{ padding: 20, textAlign: 'center', color: '#888' }}>Nadie anotado todavía.</div>}
          {ranked.map((p, i) => {
            const sc = calcScore(p.prode)
            const ch = getChampion(p.prode)
            return (
  ...
  if (screen === 'prode') {
    const tabs = [
      { id: 'groups', label: 'Grupos' }, { id: 'r32', label: '16avos' },
      { id: 'r16', label: 'Octavos' }, { id: 'qf', label: 'Cuartos' },
      { id: 'sf', label: 'Semis' }, { id: 'final', label: 'Final' }
    ]
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 11, color: '#888', marginBottom: 8 }}>
        Tocá: <span style={{ background: '#fff8e1', padding: '1px 5px', borderRadius: 4 }}>1° dorado</span>
        {' → '}
        <span style={{ background: '#e8f5e9', padding: '1px 5px', borderRadius: 4 }}>2° verde</span>
        {' → sin clasificar'}
      </div>
        ...
        {GROUPS.map(g => (
          <div key={g.name} style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: '10px', overflow: 'hidden' }}>
            ...
            {g.teams.map((t, i) => {
              ...
              const opacity = (locked && !pos) ? 0.6 : 1
              return (
                <div key={i} style={{ ...s.teamRow(pos), opacity: opacity }} onClick={() => toggle(g.name, i)}>
...
function KnockoutTab({ round, prode, setProde }) {
  ...
  function pick(matchId, team) {
    if (locked) return
    const newRound = { ...prode[round], [matchId]: team }
    setProde({ ...prode, [round]: newRound })
  }
      ...
      {Array.from({ length: count }, (_, i) => {
        ...
        return (
          <div key={id} style={{ ...s.card, marginBottom: 8 }}>
            <div style={{ background: C.red, color: '#fff', fontSize: 11, textAlign: 'center', padding: '3px' }}>
              {ROUND_LABELS[round]} {i + 1}
            </div>
            {[ta, tb].map((t, ti) => {
              const isWinner = w && t && w.n === t.n
              return (
                <div key={ti} style={{ ...s.matchTeam(isWinner), cursor: (locked || !t) ? 'default' : 'pointer' }}
                  onClick={() => { if (t && !locked) pick(id, t) }}>
                  {t
                    ? <><span style={{ fontSize: 15 }}>{t.f}</span><span>{t.n}</span>{isWinner && <span style={{ marginLeft: 'auto' }}>✓</span>}</>
...
function FinalTab({ prode, setProde }) {
  ...
  const sf0w = (prode.sf && prode.sf['sf_0']) || null
  const sf1w = (prode.sf && prode.sf['sf_1']) || null
  const qf0 = (prode.qf && prode.qf['qf_0']) || null
  const qf1 = (prode.qf && prode.qf['qf_1']) || null
  const qf2 = (prode.qf && prode.qf['qf_2']) || null
  const qf3 = (prode.qf && prode.qf['qf_3']) || null
  const loser0 = sf0w ? (sf0w.n === (qf0 && qf0.n) ? qf1 : qf0) : null
  const loser1 = sf1w ? (sf1w.n === (qf2 && qf2.n) ? qf3 : qf2) : null
  ...
  const champ = (prode.final && prode.final['final_m']) || null
    ...
    <div>
      ...
      <div style={{ ...s.card, marginBottom: 12 }}>
        ...
        {[loser0, loser1].map((t, ti) => {
          const w = (prode.third && prode.third['third']) || null
          const isWinner = w && t && w.n === t.n
          return (
            <div key={ti} style={{ ...s.matchTeam(isWinner), cursor: (locked || !t) ? 'default' : 'pointer' }}
              onClick={() => { if (t && !locked) pick('third', t, 'third') }}>
              {t
                ? <><span style={{ fontSize: 15 }}>{t.f}</span><span>{t.n}</span>{isWinner && <span style={{ marginLeft: 'auto' }}>✓</span>}</>
                : <span style={{ color: '#aaa', fontSize: 12, fontStyle: 'italic' }}>Por definir</span>}
            </div>
      ...
      <div style={{ ...s.card, border: `2px solid ${C.gold}` }}>
        <div style={{ background: C.gold, color: '#4a2800', fontSize: 12, textAlign: 'center', padding: '4px', fontWeight: 600 }}>
          FINAL MUNDIAL 2026
        </div>
        {[sf0w, sf1w].map((t, ti) => {
          const w = (prode.final && prode.final['final_m']) || null
          const isWinner = w && t && w.n === t.n
          return (
            <div key={ti} style={{ ...s.matchTeam(isWinner), cursor: (locked || !t) ? 'default' : 'pointer' }}
              onClick={() => { if (t && !locked) pick('final_m', t, 'final') }}>
              {t
                ? <><span style={{ fontSize: 15 }}>{t.f}</span><span style={{ fontSize: 14 }}>{t.n}</span>{isWinner && <span style={{ marginLeft: 'auto', fontSize: 14 }}>🏆</span>}</>
                : <span style={{ color: '#aaa', fontSize: 12, fontStyle: 'italic' }}>Por definir</span>}
            </div>
          )
        })}
